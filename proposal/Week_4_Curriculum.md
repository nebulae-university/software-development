...
sequenceDiagram
    ...
    loop Cada 50ms
        BE_Gen->>MQTT: 3. Publica evento `VehicleGenerated`
        BE_Gen-->>UI_Gen: 4. Envía dato a UI (via WebSocket)
    end
    deactivate BE_Gen

    activate BE_Rep
    BE_Rep->>MQTT: 5. Suscrito al tópico de eventos
    MQTT-->>BE_Rep: 6. Recibe eventos `VehicleGenerated`
    BE_Rep-->>BE_Rep: 7. Agrupa eventos con `bufferTime(1000)`
    loop Cada 1 segundo
        BE_Rep->>DB_Rep: 8. Procesa lote y actualiza (`findOneAndUpdate`)
        DB_Rep-->>BE_Rep: 9. DB devuelve stats actualizadas
        BE_Rep-->>UI_Rep: 10. Envía stats a UI (via WebSocket)
    end
    deactivate BE_Rep

    activate UI_Rep
    UI_Rep->>BE_Rep: 11. (Opcional) Carga inicial de datos via GraphQL
    BE_Rep->>DB_Rep: 12. Lee vista materializada
    DB_Rep-->>BE_Rep: 13. Devuelve estadísticas
    BE_Rep-->>UI_Rep: 14. Envía estadísticas al dashboard
    deactivate UI_Rep
```
...
**1. Evento de Event Sourcing**

El `ms-generator` debe publicar un evento en MQTT con la siguiente estructura:

*   `at` (Aggregate Type): Debe ser "Vehicle".
*   `et` (Event Type): Debe ser "Generated".
*   `aid` (Aggregate ID): Clave de idempotencia. Debe ser un **hash predecible** (ej. SHA-256) calculado a partir de las propiedades del vehículo en el campo `data`.
*   `timestamp`: Momento en que se generó el evento.
*   `data`: El payload del evento con los detalles del vehículo.
...
**2. Estrategia de Idempotencia y Unicidad de Vehículos**

Para cumplir con el requisito de que el `ms-reporter` no cuente el mismo vehículo dos veces, se utilizará el `aid` (el hash del vehículo) como clave de unicidad.

*   El `ms-reporter` debe mantener una colección en MongoDB, por ejemplo `processed_vehicles`, donde almacenará los `aid` (hashes) de los vehículos que ya ha contabilizado.
*   Cuando el consumidor reciba un **lote de eventos** (después del `bufferTime`), antes de procesar las estadísticas, debe filtrar este lote para descartar aquellos eventos cuyo `aid` ya exista en la colección `processed_vehicles`.
*   Después de procesar el lote de vehículos nuevos y actualizar la vista materializada de estadísticas, debe insertar los `aid` de los vehículos recién procesados en la colección `processed_vehicles`.
*   Este mecanismo asegura que si el `ms-generator` genera el mismo vehículo varias veces o si el broker MQTT entrega el mismo evento más de una vez, el vehículo solo será contabilizado una única vez en las estadísticas.
...
2.  **Backend - Lógica de Generación:**
    ...
    *   Publica un evento `VehicleGenerated` en el tópico MQTT `fleet/vehicles/generated` por cada vehículo creado.
3.  **Frontend - UI de Control y Visualización:**
    *   Implementa los botones "Iniciar" y "Detener".
    *   El backend enviará eventos al frontend a través de WebSockets cada 50ms.
    *   La UI debe actualizar los datos en memoria (RAM) con cada evento recibido.
    *   La tabla virtualizada (`react-window`) **solo debe re-renderizarse si ha pasado más de 1 segundo desde la última actualización de la UI**. Esto se puede lograr con una combinación de `useRef` para almacenar el timestamp de la última renderización y `React.memo` para controlar el re-render.
...
2.  **Backend - Consumidor por Lotes (Batch):**
    ...
    *   En la suscripción de este pipe, recibirás un array de eventos (un lote). Antes de procesarlo, debes **asegurar la idempotencia** como se definió en la "Estrategia de Idempotencia y Unicidad de Vehículos".
        1.  Filtra el lote de eventos para descartar aquellos cuyo `aid` ya exista en tu colección de `processed_vehicles`.
        2.  Si el lote filtrado está vacío, no continúes.
        3.  Itera **solo sobre el lote filtrado (vehículos nuevos)** para agregar sus datos a las estadísticas.
    *   Actualiza el documento de estadísticas en MongoDB **una sola vez** por lote usando `findOneAndUpdate` con la opción `upsert: true` y operadores como `$inc`.
    *   Inserta los `aid` de los vehículos nuevos que acabas de procesar en tu colección `processed_vehicles` para evitar que se procesen en el futuro.
    *   Después de cada actualización en la base de datos, envía las estadísticas actualizadas al frontend a través de WebSockets.
...
4.  **Frontend - Dashboard:**
    ...
    *   Al cargar, utiliza Apollo Client para ejecutar la query `getFleetStatistics` y poblar el dashboard.
    *   Escucha los eventos de WebSocket del backend para recibir las actualizaciones de las estadísticas y re-renderizar el dashboard en tiempo real.
...
| Rendimiento del Frontend | 15 | Se implementa correctamente una **lista virtualizada** (`react-window`) y se controla el re-render para que ocurra máximo una vez por segundo. |
...
| Consumidor por Lotes (Batch) con RxJS | 25 | El consumidor utiliza un `Subject` y el operador **`bufferTime(1000)`** para procesar eventos en lotes. La escritura en BD (`findOneAndUpdate`) ocurre solo una vez por segundo. |
...
| API y Dashboard en Tiempo Real | 10 | La API de GraphQL expone las estadísticas y el dashboard se actualiza en tiempo real a través de WebSockets. |
